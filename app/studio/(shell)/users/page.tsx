...
export default async function StudioUsersPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-5 py-12 sm:px-8 sm:py-14">
      <p className="mono text-[11px] uppercase tracking-[0.22em] text-white/45">Studio · Nutzer</p>
      <h1 className="mt-2.5 font-brand text-[26px] font-bold tracking-[-0.02em] text-white sm:text-[32px]">Team & Kunden</h1>
      ...
      <section className="mt-8 rounded-2xl border border-white/10 bg-white/[0.02] p-5 sm:p-6">
        ...
        <div className="mt-4 flex items-center gap-3.5">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full text-[15px] font-semibold text-white" style={{ background: "linear-gradient(135deg,#8b7bff,#39d2b4)" }}>
            {user?.imageUrl ? (
          ...
          <div className="min-w-0">
            ...
            {ownerEmail && <div className="truncate text-[13px] text-white/45">{ownerEmail}</div>}
          </div>
          <span className="mono ml-auto rounded-full border border-white/12 px-2.5 py-1 text-[10px] uppercase tracking-widest text-white/45">Inhaber:in</span>
        </div>
      ...
      <section className="mt-5 rounded-2xl border border-white/10 bg-white/[0.02] p-5 sm:p-6">
        <div className="flex items-baseline justify-between gap-4">
          <h2 className="text-[15px] font-semibold text-white">Kunden</h2>
          <span className="mono text-[11px] tracking-widest text-white/35">{clients.length}</span>
        </div>
        {clients.length === 0 ? (
          ...
          <div className="mt-4 space-y-2">
            {clients.map((c, i) => (
              <a key={i} href={`/s/${c.spaceId}/vertrag`} className="group flex items-center gap-3.5 rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3 transition-colors hover:border-white/25">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/[0.06] text-[13px] font-semibold text-white/80">
                  {(c.name || c.email || "?").slice(0, 1).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[14px] font-medium text-white">{c.name}</div>
                  <div className="truncate text-[13px] text-white/45">{c.email || "—"} · {c.projectTitle}</div>
                </div>
                {c.signed ? (
                  ...
                  <span className="mono shrink-0 rounded-full border border-white/12 px-2.5 py-1 text-[10px] uppercase tracking-widest text-white/40">Offen</span>
                )}
              </a>
            ))}
          </div>
